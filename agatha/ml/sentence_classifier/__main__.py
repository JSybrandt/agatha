...
# Train a sentence classifier to apply to agatha construction
...
from agatha.ml.train_model import train_model, split_data_across_ranks
import torch
import dask
from dask.distributed import Client
from agatha.construct.dask_checkpoint import checkpoint
from agatha.ml.sentence_classifier.util import (
    TrainingData,
    get_boundary_dates,
...
if __name__ == "__main__":
  ...
  # Used to load the sentence embedding data produced by agatha.construct
  default_ckpt_dir = (
      shared_scratch
  ...
  if not config.use_horovod or hvd.rank() == 0:
    print("Running agatha sentence_classifier with the following parameters:")
    print(config)
